...
int init(int argc, char *argv[]) {
	...
	if (argc!=4) {
		...
		cerr << "Usage: orca.exe [graphlet size: 4/5] [graph - input file] [graphlets - output file]" << endl;
		return 0;
	}
	sscanf(argv[1],"%d",&GS);
	if (GS!=4 && GS!=5) {
		cerr << "Incorrect graphlet size " << argv[1] << ". Should be 4 or 5." << endl;
		return 0;
	}
	cout << "Processing network : " << argv[2] << endl;
	...
	printf("max degree: %d\n",d_max);
	fin.close();
	// Kostya: print number of duplicate undirected edges
	int duplicate_count = m - (int)(set<PAIR>(edges,edges+m)).size();
	if (duplicate_count!=0) {
		cerr << "Input file contains " << duplicate_count << " duplicate undirected edges." << endl;
		return 0;
	}
